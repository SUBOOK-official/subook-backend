...
- 2026-09-19 추가: Meta 마케팅비·재구매 주문 비율·평균 수수료율. 데스크톱 5열 × 3행(15개), 좁은 화면 2~3열. 마케팅비는 사용자 확인에 따라 Meta만 집계하고 일별 표에서 지출을 확인한다.
...
| 재구매 주문 비율 | 현재 주문 DB의 전체 결제 이력을 구매자별 결제 시각·주문 ID로 정렬한 두 번째 이후 주문 ÷ 기간 내 전체 결제 주문 × 100. 기간 이전 이력과 환불 주문 포함, 구사이트 이력·회원/비회원 교차 식별 제외 |
| 평균 수수료율 | 환불 제외 상품의 수수료 합계 ÷ 해당 상품금액 합계 × 100. 저장된 유효 정산 스냅샷 우선, 나머지는 수거별 정책 함수(기존 30/35/40/45%, 신규 45/50%)로 예상. 전일학원은 대표가 제시한 50%를 보고용으로만 적용 |
...
수수료의 상품금액은 기존 정산과 동일하게 **주문 쿠폰·포인트 차감 전** 금액이며 배송비·박스 상품화 비용을 포함하지 않는다. 정산 전 예상액도 포함하므로 지급 완료율이나 순이익률이 아니다. 수수료 대상이 아닌 자체매입 및 전일학원 외 요율 미확인 자체판매 상품은 평균 분자·분모에서 제외하고 `commissionExcludedQuantity` 및 `commissionBreakdown`에 명시한다. 결제·정산 생성·지급 로직은 변경하지 않는다. 일별 수수료·재구매 비율도 같은 원장을 사용하고 기간 평균은 합계에서 재계산한다.
...
- `node backend/scripts/test-performance-dashboard.mjs`: PGlite 격리 PostgreSQL에서 실제 RPC와 기존 수수료 함수들을 실행한다. 운영 연결 없이 KST·환불·기존 지표·권한, 기간 이전/비회원/동시각 재구매, 6종 혼합 요율과 금액 가중 평균, 미확인 제외 및 일별 대사를 검증한다.
- SQL: 위 `test-performance-dashboard.mjs`가 최신 migration 본문과 실제 수수료 함수를 `tests/performance_dashboard.sql`에 주입해 **전체 트랜잭션을 rollback**한다. 전용 테스트 스키마만 사용하고 운영 주문/트리거에 쓰지 않는다.
