-- 2026050802의 RPC를 condition_grade 직접 전달 가능하도록 보강.
-- excel의 writingPct 컬럼에 "A+(필기율 1%)" 형태로 등급 정보가 들어있어
-- 클라이언트에서 미리 파싱한 등급을 그대로 INSERT.
...
begin
  ...
  -- 7) condition_grade NULL은 옵션 텍스트로 재계산 (fallback)
  update public.books
