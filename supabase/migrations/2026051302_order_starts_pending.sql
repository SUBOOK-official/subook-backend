-- 묶음 2: 주문 status 흐름 정상화
--
-- 변경 전: create_order가 즉시 status='paid', payment_status='paid'로 입력
--   → 계좌이체인데 입금 없이도 paid로 표시되어 settlement 트리거 발화 위험
--
-- 변경 후: status='pending', payment_status='pending'으로 시작
--   → 어드민이 카카오뱅크 거래 내역 확인 후 admin_update_order_status로 'paid' 전이
--   → 그 시점에 입금자명/금액 매칭 검증
...
begin
  ...
  -- ⚠️ status / payment_status를 'pending'으로 시작 (입금 확인 후 어드민이 paid 전이)
  insert into public.orders (
    ...
    null,  -- auto_confirm_at은 paid 전이 시점에 + delivered 후로 설정
    p_member_coupon_id,
