-- list_admin_orders RPC에 user_id 노출.
--
-- 사용자가 테스트 사이클(주문→결제→환불) 후 알림함이 비어있던 원인 중 하나.
-- AdminOrdersPage가 notifyOrderConfirmed({ order: row })을 호출하면 row.user_id가
-- undefined여서 send-notification.js의 `if (recipientUserId)` 가드에서 사이트 내
-- 알림 mirror가 통째로 skip되고 있었음.
--
-- buyer_email/buyer_name/buyer_phone은 표시용이고, user_id는 알림·권한 확인용으로
-- 별도 필요. 둘 다 jsonb에 담아준다.
