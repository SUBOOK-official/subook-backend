-- 회원 목록에 상태(정상/차단/탈퇴대기/탈퇴완료) 노출 + p_status 필터 인자 추가.
--
-- 배경: 운영자가 차단·탈퇴 회원을 목록에서 거를 수단이 없었다(P0). 상태는
--   member_profiles 컬럼으로 이미 존재(is_blocked / withdrawal_requested_at /
--   personal_data_erased_at)하지만 list_admin_members가 반환하지 않아, 프론트는
--   차단만 member_profiles에서 별도 fetch해 합치고 탈퇴는 아예 못 봤다.
--   페이지네이션이 RPC 레벨이라 클라 필터로는 현재 페이지만 걸러 부정확했다.
--
-- 변경:
--   1) members CTE에 account_status(정상/blocked/withdrawal_pending/withdrawn) 산출.
--   2) p_status 인자로 RPC 레벨 필터(=페이지네이션·카운트 정확).
--   3) rows JSON에 is_blocked·account_status·withdrawal_requested_at 노출 +
--      summary에 상태별 카운트 추가.
--   '휴면'은 명시 컬럼이 없어 이번 범위 제외(정책 정해지면 last_sign_in 기반 별도).
--
-- 시그니처가 (text,int,int) → (text,int,int,text)로 바뀌어 오버로드가 생기므로
-- 기존 함수를 drop 후 재생성하고 grant도 다시 부여한다. (본문 로직은 보존)

begin;

drop function if exists public.list_admin_members(text, integer, integer);
  ...
  select jsonb_build_object(
    ...
    'summary',
      jsonb_build_object(
        ...
        -- 상태별 카운트(현재 검색/상태 필터가 적용된 집합 기준)
...
grant execute on function public.list_admin_members(text, integer, integer, text) to authenticated;
