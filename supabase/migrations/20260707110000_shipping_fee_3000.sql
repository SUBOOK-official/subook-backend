-- 배송비 3,500원 -> 3,000원 (2026-07-07 문구 피드백 후속 — 운영 확정가).
--
-- 처음엔 윤문본의 '3,000원'을 오기로 보고 문구를 3,500원으로 맞췄으나,
-- 운영 확인 결과 3,000원이 확정가. 문구가 아니라 실제 청구 금액을 내린다.
--
-- 현재 prod의 create_order 정의(20260608010000_free_shipping_threshold_50000.sql)를
-- 그대로 가져와 v_shipping_fee 상수만 3000으로 치환(다른 로직 변경 없음).
-- 프론트 cart.js SHIPPING_FEE도 같은 커밋에서 3000으로 변경.
-- 무료배송 컷(50,000원)과 무료배송 쿠폰 로직은 v_shipping_fee를 참조하므로 자동 일관.
