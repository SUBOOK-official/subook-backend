-- 7/22 운영자 피드백 배치 B — 등록 위저드: 신규 교재 수량 + 행별 일련번호 직접 지정
--
-- 1) 신규 교재 수량 (payload: new_products[].quantity, 기본 1, 1~999 클램프)
--    증상(운영자): 같은 교재가 2권이면 옵션을 "2,2"처럼 중복 입력해야 했음.
--    옵션 배열 × 수량 만큼 books를 생성한다. 프론트 미리보기(countBooksForNewRow)와
--    권수 규칙이 반드시 일치해야 함 (옵션 수(없으면 1) × 수량).
--
-- 2) 행별 일련번호 직접 지정 (payload: new_products[].serial_override,
--    existing_additions[].serial_override — 항목 단위, 선택)
--    지정된 항목은 그 번호부터 항목 내 순서대로 배정하고, 시작 번호(serial_start)
--    순차 카운터는 지정 항목을 제외한 책만 이어서 센다 (스티커 롤은 계속 이어 쓰고,
--    지정 항목만 다른 번호대 라벨을 쓰는 운영 방식). 미지정+시작번호 없음 = 기존
--    시퀀스 자동 채번 유지.
--    검증: 배치 내 중복(친절한 메시지) → DB 사용 중 순으로 검사, 실패 시 전체 롤백.
--    최종 방어선은 부분 유니크 인덱스(idx_books_serial_number_unique).
--
-- (CREATE OR REPLACE — 시그니처 동일(p_shipment_id, p_payload), 새 키는 전부 선택이라
--  구버전 프론트 payload도 그대로 동작. 비파괴.)
